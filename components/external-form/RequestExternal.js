import { Form, FormControl, FormGroup, FormLabel, FormSelect, Button } from 'react-bootstrap';
import { useState, useEffect } from 'react';

export default function RequestDetailsSection({ formData, handleInputChange, validated }) {
    // กำหนด style สำหรับซ่อนปุ่มเพิ่ม-ลดของ input type number
    ...
    const [visitors, setVisitors] = useState(formData.visitors || [{ name: '', position: '' }]);
    
    // เริ่มต้น showImportList จาก formData.importExportOption
    const [showImportList, setShowImportList] = useState(formData.importExportOption === 'importAccess');
    
    // อัพเดต equipment จาก formData หรือใช้ค่าเริ่มต้น
    ...
    useEffect(() => {
        ...
        // อัพเดต showImportList เมื่อ importExportOption เปลี่ยน
        setShowImportList(formData.importExportOption === 'importAccess');
        
        // อัพเดต equipment เมื่อ formData.equipment เปลี่ยน
        if (formData.equipment && formData.equipment.length > 0) {
            setEquipment(formData.equipment);
        }
    }, [formData]);
    
    // เพิ่ม useEffect เฉพาะสำหรับตรวจสอบค่า importExportOption
    useEffect(() => {
        console.log('importExportOption changed:', formData.importExportOption);
        setShowImportList(formData.importExportOption === 'importAccess');
    ...
    useEffect(() => {
        const fetchCoordinators = async () => {
            ...
            try {
                ...
                const response = await fetch('/api/get-external-coord');
    ...
    const handleCheckboxChange = (e) => {
        ...
        // ส่งการเปลี่ยนแปลงไปยัง parent component ด้วยข้อมูลที่ถูกต้อง
        handleInputChange({
            target: {
                name: name,
                value: checked,
    ...
    const handleImportAccessChange = (e) => {
        ...
        console.log('Import access changed to:', isImportAccess);
        ...
        // ตรวจสอบค่าที่ส่งไป
        console.log('Sent to parent:', isImportAccess ? 'importAccess' : 'nonImportAccess');
    };
            ...
            <Form.Group className="mb-3">
                ...
                <div className="border rounded p-3 mb-2">
                    <Form.Check 
                        ...
                        id="dataCenter"
                        name="dataCenter"
                        checked={formData.dataCenter || false}
                        onChange={handleCheckboxChange}
                        style={{ color: '#85929e', fontWeight:'600' }}
                        className="mb-2"
                    />
                    <Form.Check 
                        type="checkbox" 
                        ...
                        id="supportRoom"
                        name="supportRoom"
                        checked={formData.supportRoom || false}
                        onChange={handleCheckboxChange}
                        style={{ color: '#85929e', fontWeight:'600' }}
                    />
                        ...
                        <div key={index} className="mb-3 border-bottom pb-3">
                            ...
                            <div className="d-flex mt-3">
                                ...
                                <div className="d-flex align-items-center me-4">
                                    <Form.Label className="me-0 mb-0">นำเข้า</Form.Label>
                                    <div className="d-flex align-items-center">
                                        ...
                                        <Form.Control
                                            ...
                                            required={false} // ไม่ต้องทำ validate
                                        />
                                        <Button 
                                            variant="outline-secondary" 
                                            size="sm" 
                                ...
                                {/* ส่วนนำออก */}
                                <div className="d-flex align-items-center">
                                    <Form.Label className="me-0 mb-0">นำออก</Form.Label>
                                    <div className="d-flex align-items-center">
                                        <Button 
                                            variant="outline-secondary" 
                                            size="sm" 
